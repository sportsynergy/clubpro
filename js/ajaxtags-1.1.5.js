...
//!mb
function replaceWithAttribute(sourceString, regExp, element,strAttr) {
  var retString =  sourceString.replace(regExp, element.getAttribute(strAttr));
  return retString;
}
...
AjaxJspTag.Base.prototype = {
  ...
  onRequestComplete: function(request) {
  	//lert('requestcomplete:'+request.status);
  ...
  buildParameterString: function(parameterList) {
    ...
    //!mb - modified * to *? to make it lazy 
...
AjaxJspTag.Autocomplete.prototype = (new AjaxJspTag.Base()).extend({
  ...
  //lert('autoinit');
    ...
    this.createPopup();
    this.createIframe();
    this.options.sourceElem.setAttribute("autocomplete", "on");
    //lert(this.options.sourceElem);
    //lert(this.options.sourceElem.id);
    //lert(this.options.eventType);
    this.attachBehaviors(this.options.sourceElem, this.options.eventType, this.sourceElemChanged, this);
  },
...
AjaxJspTag.Callout.prototype = (new AjaxJspTag.Base()).extend({
  ...
  sourceElemClicked: function(e) {
	...
	//!mb
	//lert(this.options.sourceAttribute);
    if (this.options.sourceAttribute==null)
	    ajaxParameters = replaceWithValue(ajaxParameters, re, this.activeElem);
	else
	    ajaxParameters = replaceWithAttribute(ajaxParameters, re, this.activeElem,this.options.sourceAttribute);
...
// some additional features by mb

function phpReplaceWithAttribute(sourceString, regExp, element,strAttr) {
  var retString =  sourceString.replace(regExp, element.getAttribute(strAttr));
  return retString;
}

/* ---------------------------------------------------------------------- */
/* extended HTML CONTENT TAG
 */

AjaxJspTag.phpHtmlContent = Class.create();
AjaxJspTag.phpHtmlContent.prototype = (new AjaxJspTag.Base()).extend({
  ...
  initialize: function(url, options) {
   ...
   getSourceElementList: function(options) {
    var list;
    if (options.sourceClass) {
      list = document.getElementsByClassName(options.sourceClass);
    } else {
  ...
  setOptions: function(options) {
    var list=this.getSourceElementList(options) ;
    this.options = {
      sourceElementList: list,
      //!mb
      sourceAttribute:null,
  ...
  sourceElemClicked: function(e) {
    ...
    if (this.options.sourceClass) {
      ...
      //!mb
      if (this.options.sourceAttribute!=null)
          ajaxParameters = phpReplaceWithAttribute(ajaxParameters, re, this.activeElem,this.options.sourceAttribute);
      else
	      ajaxParameters = replaceWithValue(ajaxParameters, re, this.activeElem);
    }
    this.setAjaxOptions({
  ...
  handlerFunction: function(xml) {
  	  //!mb reattach behaviours because html returned can contain new source elements
  	  this.options.sourceElementList=this.getSourceElementList(this.options);
	  this.attachBehaviors(this.options.sourceElementList,
                         this.options.eventType,
                         this.sourceElemClicked,
                         this);
...
//TODO:adapting tree tag from 1.3 release
/**
 * TREE TAG
 */
AjaxJspTag.Tree = Class.create();
AjaxJspTag.Tree.prototype = (new AjaxJspTag.Base()).extend({
