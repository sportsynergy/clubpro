...
?>
	...
	<a href="javascript:submitForm('exportDataForm')">Export this list</a>
		...
		while ($row = mysql_fetch_array($playerresult)) {
			...
			<form name="playerform<?=$rownum?>" method="post">
			...
			<td colspan="2">
				<div align="center">
					<a href="javascript:submitFormWithAction('playerform<?=$rownum?>','<?=$_SESSION["CFG"]["wwwroot"]?>/admin/change_settings.php')">Edit</a> | 
				  	<a href="javascript:submitFormWithAction('playerform<?=$rownum?>','<?=$_SESSION["CFG"]["wwwroot"]?>/admin/player_delete.php')">Delete</a>
				</div>
			</td>
