...
if (match_referer() && isset($_POST['courttype'])) {
	...
	if ( empty($errormsg) ){
		...
		if( isDebugEnabled(1) )
		logMessage("Inserting the reservation");
...
}else if( $usertype == 1  && isDoublesReservationNeedPlayers($time, $courtid) ) {
...
function validate_form(&$frm, &$errors) {
	...
	******************************************/
		...
		else if( $frm['action'] =="addplayerorteam" && $frm['userid'] == $frm['partner']  ){
		...
		else if ( get_roleid() == 1 && $frm['matchtype']==3 &&  $frm['action']=="addteam"  ) {
