...
function isPlayingInLadder($userid, $ladderid) {
    ...
    $query = "SELECT 1 FROM tblClubLadder WHERE userid IN ($teamINClause) AND ladderid = $ladderid AND enddate IS NULL";
    $result = db_query($query);
    $rows = mysqli_num_rows($result);
    $value = $rows > 0 ? "is" : "is not";
    
    if (isDebugEnabled(1)) logMessage("team_ladder.isPlayingInLadder: $userid  $value in the ladder for ladderid: $ladderid.");
...
function confirmChallengerTeam($my_partner, $challengees) {
    ...
    send_email($subject, $challenger_email, $content, "Team Ladder");
...
function confirmChallengeeTeam($challengees, $message) {
    ...
    //Send the email
    send_email($subject, $challengee_email, $content, "Team Ladder");
}
?>