...
class UpdateBoxLeagueScores{
	...
	public function updateScores(){
        ...
        if (isDebugEnabled(2)) logMessage("UpdateBoxLeagueScores: Starting...");
        ...
        while($box_array = mysqli_fetch_array($mresult) ){
            ...
            if (isDebugEnabled(2)) logMessage("UpdateBoxLeagueScores: Processing ". $box_array['boxname'] . "(".$box_array['ladderid'].") for $count players"); 
          

            /* 3.) for each player in the ladder figure out the scores   */
            while($lm_player_array = mysqli_fetch_array($lmresult) ){

                if (isDebugEnabled(2)) logMessage("UpdateBoxLeagueScores: ---------------------------------" ); 
                if (isDebugEnabled(2)) logMessage("UpdateBoxLeagueScores: Calculating scores for ". $lm_player_array['playername']." (".$lm_player_array['player'].")" ); 
                if (isDebugEnabled(2)) logMessage("UpdateBoxLeagueScores: ---------------------------------" ); 
                ...
                while($lp_match_array = mysqli_fetch_array($lpresult) ){
                    ...
                    if( $lm_player_array['player'] == $lp_match_array['winnerid']){
                        ...
                        if ($scores[0] == '3'){
                            ...
                            if (isDebugEnabled(2)) logMessage("UpdateBoxLeagueScores: Not a valid score ".$lp_match_array['score']  ); 
                        }
                        ...
                        if($pastmatches == 0){
                            ...
                            if (isDebugEnabled(2)) logMessage("UpdateBoxLeagueScores: Adding 2 points for a win (".$lp_match_array['score'].") in first match against ".$lp_match_array['loser']." on ".$lp_match_array['match_time'] . " and updated gameswon to $gameswon" );   
                            ...
                            if (isDebugEnabled(2)) logMessage("UpdateBoxLeagueScores: Adding 1 points for a win (".$lp_match_array['score'].") in a rematch against ".$lp_match_array['loser']." on ".$lp_match_array['match_time']. " and updated gameswon to $gameswon" );  


                        } else {

                            if (isDebugEnabled(2)) logMessage("UpdateBoxLeagueScores: no scoring for a win after more than 2 matches against ".$lp_match_array['loser']." on ".$lp_match_array['match_time'] ); 
                        }
                    ...
                    if( $lm_player_array['player'] == $lp_match_array['loserid']){
                        ...
                        if($pastmatches == 0){
                            ...
                            if (isDebugEnabled(2)) logMessage("UpdateBoxLeagueScores: Adding 1 point for a loss (".$lp_match_array['score'].") in first match against ".$lp_match_array['winner'] ." on ".$lp_match_array['match_time']. " and updated gameswon to $gameswon" );  
                            ...
                            if (isDebugEnabled(2)) logMessage("UpdateBoxLeagueScores: Adding 0 points for a loss (".$lp_match_array['score'].") in a rematch against ".$lp_match_array['winner'] ." on ".$lp_match_array['match_time']. " and updated gameswon to $gameswon" ); 

                        } else {

                            if (isDebugEnabled(2)) logMessage("UpdateBoxLeagueScores: no scoring for a loss after more than 2 matches against ".$lp_match_array['winnerid'] ." on ".$lp_match_array['match_time'] ); 
                        }
                ...
                if (isDebugEnabled(2)) logMessage("UpdateBoxLeagueScores: Total points for ".$lm_player_array['playername'] . " is: $points and total games is $games and gameswon is $gameswon for box ".$box_array['boxid'].".  " ); 
