	...
	public function updateScores(){
        ...
        while($box_array = mysqli_fetch_array($mresult) ){
            ...
            while($lm_player_array = mysqli_fetch_array($lmresult) ){
                ...
                $query = "SELECT * FROM tblLadderMatch where winnerid = ".$lm_player_array['player']." or loserid = ".$lm_player_array['player'];
                ...
                while($lp_match_array = mysqli_fetch_array($lpresult) ){
                    ...
                    if( $lm_player_array['player'] == $lp_match_array['winnerid']){
                        ...
                        if($pastmatches == 0){
                            ...
                            if (isDebugEnabled(1)) logMessage("UpdateBoxLeagueScoresService:updateScores. Adding 2 points for a win in first match" ); 

                            $points = $points + 2;

                        } elseif($pastmatches == 1){

                            if (isDebugEnabled(1)) logMessage("UpdateBoxLeagueScoresService:updateScores. Adding 1 points for a win in a rematch" ); 

                            $points = $points + 1;

                        } else {

                            if (isDebugEnabled(1)) logMessage("UpdateBoxLeagueScoresService:updateScores. no scoring for a win after more than 2 matches" ); 
                        }
                    ...
                    if( $lm_player_array['player'] == $lp_match_array['loserid']){
                        ...
                        if($pastmatches == 0){
                            ...
                            if (isDebugEnabled(1)) logMessage("UpdateBoxLeagueScoresService:updateScores. Adding 1 point for a loss in first match" ); 

                            $points = $points + 1;

                        } elseif($pastmatches == 1){

                            if (isDebugEnabled(1)) logMessage("UpdateBoxLeagueScoresService:updateScores. Adding 0 points for a loss in a rematch" ); 

                        } else {

                            if (isDebugEnabled(1)) logMessage("UpdateBoxLeagueScoresService:updateScores. no scoring for a loss after more than 2 matches" ); 
                        }

                        array_push($players, $lp_match_array['winnerid']);
                    }

                    if (isDebugEnabled(1)) logMessage("UpdateBoxLeagueScoresService:updateScores. total points for user ".$lm_player_array['player'] . " is: $points and total games is $games" ); 
                    ...
                    if ( isset($boxid) ) {
                        ...
                        $query = "UPDATE tblkpBoxLeagues SET score = $points, games = $games WHERE boxid = $boxid AND userid = ".$lm_player_array['player'];
