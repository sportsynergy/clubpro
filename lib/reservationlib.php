...
function isAtLeastOnePlayerDuplicatedForDoubles($playerOne, $playerTwo, $playerThree, $playerFour){
	...
	//Check Player One
	if(!isClubGuest($playerOneTrimmed) 
	&& isPlayerSpecified($playerOneTrimmed)
	&& ($playerOneTrimmed == $playerTwoTrimmed || $playerOneTrimmed == $playerThreeTrimmed || $playerOneTrimmed == $playerFourTrimmed)){
		return true;
	}

	//Check Player Two
	else if(!isClubGuest($playerTwoTrimmed) 
	&& isPlayerSpecified($playerTwoTrimmed)
	&& ($playerTwo == $playerOneTrimmed || $playerTwoTrimmed == $playerThreeTrimmed || $playerTwoTrimmed == $playerFourTrimmed)){
		return true;
	}
	
	//Check Player Three
	else if(!isClubGuest($playerThreeTrimmed) 
	 && isPlayerSpecified($playerThreeTrimmed)
	&& ($playerThree == $playerOneTrimmed || $playerThreeTrimmed == $playerTwoTrimmed || $playerThreeTrimmed == $playerFourTrimmed)){
		return true;
	}
	
	//Check Player Four
	else if(!isClubGuest($playerFourTrimmed) 
	&& isPlayerSpecified($playerFourTrimmed)
	&& ($playerFourTrimmed == $playerOneTrimmed || $playerFourTrimmed == $playerTwoTrimmed || $playerFourTrimmed == $playerThreeTrimmed)){
		return true;
	}
...
function confirmCourtEvent($userid, $reservationid, $action, $adminaction){
	...
	$to_email = array($var->email => array('name' => $var->firstname));
	$content = new Object;
	...
	//Set Email Variables
	$to_email = array($var->adminemail => array('name' => $var->adminfirstname) );
	$content = new Object;
	$content->line1 = $emailbody;
	$content->clubname = get_clubname();
	...
	while ($player = mysql_fetch_array($rresult)) {
		...
		if (!empty ($player['email']) && $player['userid']!=$userid ) {
			...
			$to_emails[$player['email'] = array('name' => $player['firstname'])];
