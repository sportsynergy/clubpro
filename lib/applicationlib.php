...
/**
 * Login Function.  Sets the view in the session for which the user will use. 
 */
function set_view() {
...
function email_players($resid, $emailType) {
	...
	if ($usertypeval == 0) {
		...
		$rquery = "SELECT courts.courtname, reservations.time, users.firstname, users.lastname, courttype.courttypeid, rankings.ranking,  users.email, users.homephone, users.cellphone, users.workphone, matchtype.name
...
function report_scores_singles_simple($wUserid, $lUserid, $wor, $wnr, $lor, $lnr, $score) {
...
function report_scores_doubles_simple($wTeamid, $lTeamid, $wor, $wnr, $lor, $lnr, $score) {
...
function record_score(& $frm) {
	...
	if ($usertypeval == 1) {
		...
		//For winner team
		$winners = getUserIdsForTeamId($winner);
	   ...
	   $playerTwoRankQuery = "SELECT rankings.ranking 
		...
		//For loser team
		$losers = getUserIdsForTeamId($loser);	
...
function get_admin_player_search($searchname) {
	...
	$playerquery = "SELECT users.firstname, users.lastname, users.email, users.workphone, users.homephone, users.userid
	                        FROM tblUsers users, tblClubUser clubuser
	                        WHERE users.userid = clubuser.userid
							AND (((clubuser.clubid)=" . get_clubid() . "))
...
function get_player_search($searchname) {
	...
	$playerquery = "SELECT user.userid, user.firstname, user.lastname, user.email, user.workphone, user.homephone
...
function getUserIdsForTeamId($teamid) {
	...
	$playersQuery = "SELECT tblkpTeams.userid
	                   FROM tblkpTeams
	                   WHERE (((tblkpTeams.teamid)=$teamid)) ";
	...
	while ($team = mysql_fetch_array($playersResult)) {
		array_push($teams, $team['userid']);
	}
	return $teams;
 ...
 */
 function isSiteCourtSportEnabled(){
 	

 	if( in_array( "court_sport", $_SESSION["siteattribs"] )){
 		return true;
 	}
 	else{
 		return false;
 ...
 */
 function isSiteBoxLeageEnabled(){
 	...
 	if(  in_array( "web_ladder" , $_SESSION["siteattribs"])){
 		return true;
 	}
 	else{
 		return false;
 ...
 */
 function getFullNamesForTeamId($teamId){
    ...
    return getFullNameForUserId($playerOne)." / ".getFullNameForUserId($playerTwo);
